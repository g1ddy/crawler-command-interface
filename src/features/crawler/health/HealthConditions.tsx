...
export function HealthConditions({ state, observations, onInspectObservation }: { state: CrawlerState; observations: ProjectedObservationsState; onInspectObservation: (o: ProjectedObservationValue) => void }) {
  ...
  const renderEffects = (effects: CrawlerState["effects"], className?: string) => effects.length ? effects.map(e => <p key={e.effectId} className={className}>{e.icon} {e.name} <b>{e.durationSeconds}s</b><small>{e.description}</small></p>) : <p>NONE</p>;
