...
export function RatingsView({ observations, selectedSequence, isLive, onInspectObservation }: { observations: Record<string, ProjectedObservationValue>; selectedSequence?: number; isLive: boolean; onInspectObservation: (o: ProjectedObservationValue) => void }) {
  ...
  return <section className="view-content"><header className="title"><div><p className="eyebrow">BROADCAST TELEMETRY</p><h1>RATINGS</h1></div><b>{isLive ? "● LIVE AUDIENCE" : "◷ REPLAY AUDIENCE"}{viewers ? ` ${viewers.value.toLocaleString()}` : ""}</b></header>{metrics.length === 0 ? <Panel title="RATINGS UNAVAILABLE"><p>No authored broadcast metrics are available at this sequence.</p></Panel> : <div className="broadcast-page">{activeGroups.map(group => <Panel key={group} title={headings[group]}><div className="audience">{metrics.filter(metric => metric.group === group).map(metric => <p key={metric.label} style={{ display: "flex", justifyContent: "space-between" }}><span><b>{typeof metric.value === "number" ? metric.value.toLocaleString() : metric.value}</b> {metric.label}</span><TelemetryBadge observation={metric.observation} selectedSequence={selectedSequence} onClick={() => onInspectObservation(metric.observation)} /></p>)}</div></Panel>)}</div>}</section>;
}
