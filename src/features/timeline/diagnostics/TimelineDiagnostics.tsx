...
export function TimelineDiagnostics({ events, observations, selectedSequence, minSequence, maxSequence, projectedObservations, onSelectSequence, onInspectObservation }: {
  ...
  return <div style={{ marginTop: "14px", borderTop: "1px solid #183e4d", paddingTop: "10px" }}>
    ...
    {showDiagnostics && <div className="timeline-meta" style={{ flexDirection: "column", alignItems: "stretch", gap: "12px", marginTop: "12px" }}>
      ...
      <div className="slider-wrapper"><div className="event-markers">{markerEvents.map((event) => { const pct = ((event.sequence - minSequence) / (maxSequence - minSequence || 1)) * 100; const narrativePresentation = event.type === "NarrativeEvent" ? getNarrativePresentation(String(event.kind)) : null; return <button key={`event-${event.sequence}`} style={{ left: `${pct}%` }} className={`marker marker-${event.category} ${narrativePresentation ? `typed-marker marker-${narrativePresentation.group} ${narrativePresentation.terminal ? "terminal" : ""}` : ""} ${event.sequence === selectedSequence ? "active" : ""}`} onClick={() => onSelectSequence(event.sequence)} onMouseEnter={() => setHoveredEvent(event)} onMouseLeave={() => setHoveredEvent(null)} aria-label={narrativePresentation ? `${narrativePresentation.accessibleLabel}: ${event.summary}` : event.summary} />; })}{markerObservations.map((observation) => { const pct = ((observation.sequence - minSequence) / (maxSequence - minSequence || 1)) * 100; return <button key={`observation-${observation.id}`} style={{ left: `${pct}%` }} className={`marker obs-marker ${observation.sequence === selectedSequence ? "active" : ""}`} onClick={() => onSelectSequence(observation.sequence)} onMouseEnter={() => setHoveredObservation(observation)} onMouseLeave={() => setHoveredObservation(null)} title={`[Sourced Telemetry · Seq #${observation.sequence}] ${observation.kind}`} />; })}</div></div>
