...
export function EquipmentView({ state, liveState, observations, selectedSequence, isLive = true, slot, setSlot, actions, onOpenProvenance, onInspectObservation }: {
  state: CrawlerState;
    ...
    <div style={{ display: "grid", gap: "18px" }}>
      ...
      {activeCandidate && <Panel title="GEAR STAT COMPARISON & DELTAS">
        ...
        <div className="actions" style={{ flexWrap: "wrap", gap: "6px" }}>
          {activeCandidate.instanceId !== equippedItem?.instanceId && <button style={{ background: isLive && requirements.met ? "#0e3a24" : "#2a1818", borderColor: isLive && requirements.met ? "#2de079" : "#633030", color: isLive && requirements.met ? "#62ef98" : "#8a5858", cursor: isLive && requirements.met ? "pointer" : "not-allowed" }} disabled={!isLive || !requirements.met} onClick={() => isLive && requirements.met && actions.equipItem(activeCandidate.instanceId, slot)}>EQUIP GEAR ⚔</button>}
          {activeCandidate.durability && activeCandidate.durability.current < activeCandidate.durability.max && <button style={{ background: isLive ? "#212d12" : "#141c10", borderColor: isLive ? "#86c934" : "#3b581e", color: isLive ? "#bcf26d" : "#5d783d", cursor: isLive ? "pointer" : "not-allowed" }} disabled={!isLive} onClick={() => isLive && actions.repairItem(activeCandidate.instanceId)}>REPAIR 🛠</button>}
          <button style={{ background: isLive ? "#0e2330" : "#111a24", borderColor: isLive ? "#30729e" : "#234054", color: isLive ? "#86cbff" : "#51748f", cursor: isLive ? "pointer" : "not-allowed" }} disabled={!isLive} onClick={() => isLive && actions.toggleItemLock(activeCandidate.instanceId)}>{activeCandidate.isLocked ? "UNLOCK 🔒" : "LOCK 🔓"}</button>
          ...
          {!activeCandidate.isEquipped && <button style={{ background: !isLive || activeCandidate.isLocked ? "#201214" : "#2e1215", borderColor: !isLive || activeCandidate.isLocked ? "#4d2226" : "#d14b54", color: !isLive || activeCandidate.isLocked ? "#6e4246" : "#ff8a90", cursor: !isLive || activeCandidate.isLocked ? "not-allowed" : "pointer" }} disabled={!isLive || activeCandidate.isLocked} onClick={() => isLive && !activeCandidate.isLocked && actions.discardItem(activeCandidate.instanceId)}>DISCARD 🗑️</button>}
        </div>
      </Panel>}
    </div>
  </div>;
