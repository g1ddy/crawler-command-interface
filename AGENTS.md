...
3. **Authoritative Raw Evidence**: Story evidence is authored strictly under `data/raw/` (`catalogs/` and `floors/floor-*/`). `data/floors/*.json` and `data/compiled-timeline.json` are generated with `npm run generate:fixture` rather than hand-edited.
...
| Raw Floor Loader & Assembler | `app/domain/raw-loader.ts` |
| Compiler & Raw Adapter | `app/domain/raw-compiler.ts`, `app/domain/compiler.ts` |
| Decomposed Raw Story Evidence | `data/raw/catalogs/`, `data/raw/floors/floor-*/` |
