...
8. **Generated Visual Documentation and Architecture Evidence**: `docs/images/screenshot-*.png`, `.maritime/*`, and `docs/images/dependency-graph.svg` are generated, tracked canonical repository assets. Do not hand-edit or retouch them. PR verification workflows (`ci.yml`, `screenshots.yml`, `maritime-analysis.yml`) run automatically as read-only checks without writing back to the active PR branch. Artifact publication is controlled by an explicit, approval-gated workflow (`publish-artifacts.yml`) protected by the `artifact-finalization` environment.
...
`npm run verify` runs fixture generation, linting, unit tests, both deployment builds, rendered preview tests, and artifact contract checks. Documentation screenshot generation and Maritime architecture verification run as separate, read-only PR workflows (`screenshots.yml` and `maritime-analysis.yml`). They upload generated artifacts for inspection without mutating the PR branch head. Maintainers promote generated artifacts using the approval-gated `publish-artifacts.yml` workflow (`artifact-finalization` environment), which verifies head SHA freshness before creating a single combined artifact commit.
