...
8. **Generated Visual Documentation and Architecture Evidence**: `docs/images/screenshot-*.png`, `.maritime/*`, and `docs/images/dependency-graph.svg` are generated, tracked canonical repository assets. Do not hand-edit or retouch them. Verification jobs run automatically as read-only checks without writing back to the active PR branch. Screenshot and Maritime updates are promoted together by the single approval-gated `publish` job in `publish-artifacts.yml`, protected by the `artifact-finalization` environment; its generated-only commit uses `[skip ci]` so publication does not recursively start another PR verification cycle.
