...
2. **Generated Floor Compatibility Documents (`data/floors/*.json`)**:
   - Transitional compatibility scaffolding that mirrors floor-scoped authoring documents.
   - Preserved as a durable contract for backward compatibility with tooling or external consumers until a full deprecation migration is initiated.
