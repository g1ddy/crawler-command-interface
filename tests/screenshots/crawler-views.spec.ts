...
test("quest filter falls back when UNKNOWN disappears during replay", async ({ page }) => {
  await page.evaluate(() => {
    const replayQuestDoc = {
      schemaVersion: "crawler-timeline/v1",
      timeline: {
        id: "tl-quest-replay",
        title: "Quest Replay Timeline",
        story: { id: "st-quest-replay", title: "Quest Replay Story" },
      },
      sources: [
        {
          id: "src-quest-replay",
          kind: "official-text",
          trust: "primary",
          title: "Quest Replay Source",
          url: "https://example.com/quest-replay",
        },
      ],
      initialState: {
        crawler: {
        ...
        quests: [
          {
            ...
            title: "Replay Quest",
            urgency: "STANDARD",
            goals: ["Observe the status transition"],
            rewards: "None",
          },
        ],
      },
      events: [
        {
          ...
          position: { floor: 1 },
          summary: "Entered Floor 1",
          evidence: [{ sourceId: "src-quest-replay" }],
        },
        {
          id: "evt-replay-quest-active",
          sequence: 2,
          type: "QuestUpdated",
          questId: "q-replay",
          title: "Replay Quest",
          ...
          rewards: "None",
          status: "active",
          summary: "Replay Quest became active",
        },
      ],
    };
    localStorage.setItem("crawler_timeline_doc_v2", JSON.stringify(replayQuestDoc));
  });
  await page.reload();
  await expect(page.getByText("FLOOR NAVIGATOR:")).toBeVisible();
  ...
  const unknownFilter = page.getByRole("button", { name: /^UNKNOWN\b/ });
  await expect(unknownFilter).toBeVisible();
  await unknownFilter.click();
  await expect(unknownFilter).toHaveClass(/\bon\b/);

  await page.getByRole("slider", { name: "Selected timeline sequence" }).fill("2");
  await expect(page.getByRole("button", { name: /^UNKNOWN\b/ })).toHaveCount(0);
  await expect(page.getByRole("button", { name: /^ACTIVE\b/ })).toHaveClass(/\bon\b/);
  await expect(page.getByRole("heading", { name: "Replay Quest", exact: true })).toBeVisible();
});
