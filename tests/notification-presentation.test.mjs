...
const events = [
  { ...base, id: "achievement", sequence: 1, type: "AchievementUnlocked", summary: "Unlocked One" },
  { ...base, id: "narrative", sequence: 2, type: "NarrativeEvent", kind: "floor-collapsed", summary: "A dramatic collapse" },
  { ...base, id: "level", sequence: 3, type: "LevelChanged", level: 2, summary: "Reached level two" },
...
test("notification projection is semantic and replay-bounded", () => {
  assert.deepEqual(projectNotifications(events, state, 2).map(({ kind, sequence }) => ({ kind, sequence })), [
    { kind: "achievement", sequence: 1 },
  ]);
  assert.deepEqual(projectNotifications(events, state, 4).map(item => item.kind), ["progression", "achievement"]);
});

test("notification kind remains separate from severity", () => {
  const [notice] = projectNotifications(events, state, 1);
