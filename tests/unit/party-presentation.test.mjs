...
test("derivePartyPresentation handles non-standard or unknown member roles safely", () => {
  const partyWithUnknownRole = {
    partyId: "party-test",
    name: "Test Guild",
    members: [
      { crawlerId: "crawler-1", name: "Adventurer One", role: "scout" },
    ],
  };

  const pres = derivePartyPresentation({ party: partyWithUnknownRole });
  const [member] = pres.members;
  assert.equal(member.isLeader, false);
  assert.equal(member.roleLabel, "MEMBER");
});
